...
try:
    ...
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "odfpy", "--break-system-packages"]
    )
    from odf.opendocument import OpenDocumentSpreadsheet
    from odf.style import (
        Style,
...
def create_styles(doc, currency_symbol="₴"):
    ...
    negative_style = Style(
        name="NegativeStyle", family="table-cell", datastylename="CurrencyStyle"
    )
...
def create_budget_from_json(json_file):
    ...
    if "incomes" in data and data["incomes"]:
        ...
        current_row = (
            income_total_row + 2
        )  # +1 за пустую строку, +1 для следующего заголовка
    ...
    if "savings" in data and data["savings"]:
        savings_total_row = add_section(
            table, "НАКОПЛЕНИЯ И ЦЕЛИ", data["savings"], current_row, styles, currency
        )
    ...
    if expense_cells:
        ...
        total_expenses_row.addElement(
            create_cell("", formula=total_expenses_formula, style=styles["total"])
        )
    else:
        total_expenses_row.addElement(
            create_cell(0, value=0, style=styles["total"], value_type="float")
        )
    ...
    if "income" in total_rows:
        ...
        balance_row.addElement(
            create_cell("", formula=balance_formula, style=styles["total"])
        )
    else:
        balance_row.addElement(
            create_cell(0, value=0, style=styles["total"], value_type="float")
